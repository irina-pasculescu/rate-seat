...
class RequestTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    public function testConstructorBuildsRequestWithArrayHeaders()
    {
        ...
        $request = new Request(
            'GET', 'http://www.guzzle-project.com/', array(
            'foo' => 'bar'
        )
        );
    ...
    {
        $request = new Request(
            'GET', 'http://www.guzzle-project.com/', new Collection(
            array(
                'foo' => 'bar'
            )
        )
    ...
    public function testRequestsCanBeConvertedToRawMessageStrings()
    {
        ...
        $request = RequestFactory::getInstance()->create(
            'PUT', 'http://www.google.com/path?q=1&v=2', array(
                     'Authorization' => 'Basic ' . $auth
                 ), 'Data'
        );
    ...
    public function testCanShortCircuitErrorHandling()
    {
        ...
        $request->getEventDispatcher()->addListener(
            'request.error', function ( $event ) use ( &$out, $that ) {
                $out .= $event[ 'request' ] . "\n" . $event[ 'response' ] . "\n";
    ...
    public function testCanOverrideUnsuccessfulResponses()
    {
        ...
        $request = $this->request;
        $request->getEventDispatcher()->addListener(
            'request.error', function ( $event ) use ( &$newResponse ) {
    ...
    public function testCanSendCustomRequests()
    {
        ...
        $request  = $this->client->createRequest(
            'PROPFIND', $this->getServer()->getUrl(), array(
