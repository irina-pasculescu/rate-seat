...
class HttpRequestFactoryTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    public function testCreatesNewPostRequestWithFields()
    {
        ...
        $request = RequestFactory::getInstance()->create(
            'POST', 'http://www.google.com/path?q=1&v=2', null, array(
                      'a' => 'b'
                  )
        );
        ...
        // Use a collection
        $request = RequestFactory::getInstance()->create(
            'POST', 'http://www.google.com/path?q=1&v=2', null, new Collection(
                      array(
                          'a' => 'b'
                      )
                  )
        ...
        // Use a QueryString
        $request = RequestFactory::getInstance()->create(
            'POST', 'http://www.google.com/path?q=1&v=2', null, new QueryString(
                      array(
                          'a' => 'b'
                      )
                  )
        ...
        $request = RequestFactory::getInstance()->create(
            'POST', 'http://www.test.com/', null, array(
                      'a'    => 'b',
                      'file' => '@' . __FILE__
                  )
        );
    ...
    {
        $request = RequestFactory::getInstance()->create(
            'GET', 'http://foo.com', array(), null, array(
                     'query' => array( 'Foo' => 'Bar' )
                 )
        );
    ...
    public function testCanSetDefaultQueryString()
    {
        ...
        RequestFactory::getInstance()->applyOptions(
            $request, array(
    ...
    public function testCanAddEvents()
    {
        ...
        $request = $client->get(
            $this->getServer()->getUrl(), array(), array(
                                            'events' => array(
